...
async function createNodeImportMap(options: InitNodeFederationOptions): Promise<ImportMap> {
...
async function loadFsManifest(manifestUrl: string): Promise<Record<string, string>> {
  const content = await fs.readFile(manifestUrl, 'utf-8');
  const manifest = JSON.parse(content) as Record<string, string>;
  return manifest;
}

async function loadFsFederationInfo(relBundlePath: string): Promise<FederationInfo> {
